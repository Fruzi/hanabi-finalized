...
class ObservationVectorizer(object):
    ...
    def vectorize_observation(self, obs):
        ...
        self.last_player_action = None
        if obs["last_moves"] != []:
            for i in range(0, len(obs['last_moves'])):
                if obs["last_moves"][i].move().type() != DEAL and obs["last_moves"][i].move().type() != TURN:
                    self.last_player_action = obs['last_moves'][i]
                    break
        ...
        self.encode_last_action(obs['current_player'])
    ...
    def encode_last_action(self, current_player):
        if self.last_player_action is None:
            ...
            been me (player id 0).
            '''
            action_taker_offset = (self.last_player_action.player() - current_player) % self.num_players
            self.obs_vec[self.offset + action_taker_offset] = 1
            self.offset += self.num_players
            # encode move type
            ...
            if last_move_type == REVEAL_COLOR or last_move_type == REVEAL_RANK:
                target = (
                                     self.last_player_action.player() + self.last_player_action.move().target_offset()) % self.num_players
                target_offset = (target - current_player) % self.num_players
                self.obs_vec[self.offset + target_offset] = 1
            else:
                self.obs_vec[self.offset + action_taker_offset] = 1
            self.offset += self.num_players
    ...
    def encode_card_knowledge(self, obs):
        ...
        for pid, player_card_knowledge in enumerate(card_knowledge_list):
            ...
            for card_id, card in enumerate(player_card_knowledge):
                ...
                if card['explicit_color'] is not None:
                    color = card['explicit_color']
                    self.obs_vec[self.offset + color] = 1
                self.offset += self.num_colors

                if card["explicit_rank"] is not None:
                    rank = card["explicit_rank"]
...
class LegalMovesVectorizer(object):
    ...
    def get_move_uid(self, move):
        if move["action_type"] == "DISCARD":
            ...
            rank = move["rank"]
            target_offset = move["target_offset"]
            return self.hand_size + self.hand_size + self.max_reveal_color_moves + (
