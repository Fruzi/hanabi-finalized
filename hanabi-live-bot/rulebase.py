...
# Note: depending on the object calling, card could either be a dict eg {'color':'R','rank':0} or a HanabiCard
# instance with c.color() and c.rank() methods
def playable_card(card, fireworks):
    if isinstance(card, pyhanabi.HanabiCard):
        card = {'color': colors[card.color()], 'rank': card.rank()}

    """A card is playable if it can be placed on the fireworks pile."""
    if card['color'] is None and card['rank'] is not None:
        for color in colors:
        ...
        return True
    elif card['color'] is None or card['rank'] is None:
        return False
    else:
...
def min_on_table(fireworks):
    min_val = 5
    for color in fireworks:
        if fireworks[color] < min_val:
            min_val = fireworks[color]
    return min_val


def is_safe_to_discard(card, fireworks, max_fireworks):
    if isinstance(card, pyhanabi.HanabiCard):
        card = {'color': colors[card.color()], 'rank': card.rank()}
    if card['color'] is not None:
        if card['rank'] is not None:
...
class Ruleset:
        ...
        if observation['information_tokens'] > 0:
            ...
            for index, card in enumerate(their_knowledge):
                if card['color'] is None:
                    return {'action_type': 'REVEAL_COLOR', 'color': their_hand[index]['color'],
                            'target_offset': PLAYER_OFFSET}
                if card['rank'] is None:
                    return {'action_type': 'REVEAL_RANK', 'rank': their_hand[index]['rank'],
                            'target_offset': PLAYER_OFFSET}
        return None
    ...
    # Prioritizes Rank
    @staticmethod
    def tell_anyone_about_oldest_useful_dard(observation):
        fireworks = observation['fireworks']
                        ...
                        for card, hint in zip(player_hand, player_hints):
                            ...
                            if rank < fireworks[color]:
                                if known_color is None and known_rank is not None:
                                    return {'action_type': 'REVEAL_COLOR', 'color': color,
                                            'target_offset': player_offset}
    ...
    @staticmethod
    def tell_anyone_ones(observation):
        if observation['information_tokens'] > 0:
            for player_offset in range(1, observation['num_players']):
                player_hand = observation['observed_hands'][player_offset]
                player_hints = observation['card_knowledge'][player_offset]
                for card, hint in zip(player_hand, player_hints):
                    if card['rank'] == 0:
                        ...
                        if hint['color'] is None:
                            return {'action_type': 'REVEAL_COLOR', 'color': card['color'],
                                    'target_offset': player_offset}
        return None
    ...
    def tell_anyone_useless_card(observation):
        ...
        if observation['information_tokens'] > 0:
            ...
            for player_offset in range(1, observation['num_players']):
                ...
                player_hints = observation['card_knowledge'][player_offset]
                for card, hint in zip(player_hand, player_hints):
                    if is_safe_to_discard(card, fireworks, max_fireworks):
                        if hint['rank'] is None:
                            return {'action_type': 'REVEAL_RANK', 'rank': card['rank'], 'target_offset': player_offset}
                        if hint['color'] is None:
                            return {'action_type': 'REVEAL_COLOR', 'color': card['color'],
    ...
    @staticmethod
    def tell_anyone_about_useless_card(observation):
        fireworks = observation['fireworks']
        if observation['information_tokens'] > 0:
            max_fireworks = get_max_fireworks(observation)
            for player_offset in range(1, observation['num_players']):
            ...
            for player_offset in range(1, observation['num_players']):
                ...
                for card, hint in zip(player_hand, player_hints):
                    ...
                    if affected_colors > max_affected:
                        ...
                        best_action = {'action_type': 'REVEAL_COLOR', 'color': card['color'],
                                       'target_offset': player_offset}
                    if affected_ranks > max_affected:
                        max_affected = affected_ranks
                        best_action = {'action_type': 'REVEAL_RANK', 'rank': card['rank'],
                                       'target_offset': player_offset}
            return best_action
        return None
    ...
    @staticmethod
    def discard_highest(observation):
        if observation['information_tokens'] < 8:
            player_offset = 0
            hand = observation['card_knowledge'][player_offset]
            discard_index = -1
            highest = -1
            for ind, card in enumerate(hand):
                if card['rank'] is not None and card['rank'] > highest:
                    highest = card['rank']
                    discard_index = ind
            if discard_index > -1:
    ...
    @staticmethod
    def discard_oldest_no_info(observation):
        if observation['information_tokens'] < 8:
            player_offset = 0
            hand = observation['card_knowledge'][player_offset]
            for ind, card in enumerate(hand):
                if card['rank'] is None and card['color'] is None:
                    return {'action_type': 'DISCARD', 'card_index': ind}
        return None

    @staticmethod
    def discard_if_certain(observation):
        if observation['information_tokens'] < 8:
            fireworks = observation['fireworks']
            player_offset = 0
            hand = observation['card_knowledge'][player_offset]
    ...
    @staticmethod
    def discard_least_likely_to_be_be_necessary(observation):
        if observation['information_tokens'] < 8:
            player_offset = 0
            hand = observation['card_knowledge'][player_offset]
            worst = 100
            discard_index = -1
            probabilities = get_probability_useless(observation, 0)
            for ind, card in enumerate(hand):
                if probabilities[ind] <= worst:
                    worst = probabilities[ind]
                    discard_index = ind
            if discard_index > -1:
                return {'action_type': 'DISCARD', 'card_index': discard_index}
        return None
    ...
    @staticmethod
    def discard_safe_card(observation):
        if observation['information_tokens'] < 8:
            player_offset = 0
            hand = observation['card_knowledge'][player_offset]
            fireworks = observation['fireworks']
    ...
    @staticmethod
    def discard_useless_card(observation):
        if observation['information_tokens'] < 8:
            player_offset = 0
            hand = observation['card_knowledge'][player_offset]
            fireworks = observation['fireworks']
    ...
    @staticmethod
    def if_rule(condition, function):
        if condition():
            return function()
        return None
    ...
    @staticmethod
    def smarter_hail_mary(observation):
        if observation['deck_size'] == 0 and observation['life_tokens'] > 1:
            return Ruleset.play_probably_safe_factory(0.1)(observation)

    # Plays the card that the agent has the most information about
    @staticmethod
    def play_most_info(observation):
        hand = observation['card_knowledge'][0]
        most_info = 0
        index = 0
        for ind, hint in enumerate(hand):
            knowledge = 0
            if hint['color'] is not None:
                knowledge = knowledge + 1
            if hint['rank'] is not None:
                knowledge = knowledge + 1
            if knowledge > most_info:
                most_info = knowledge
                index = ind
        action = {'action_type': 'PLAY', 'card_index': index}
        return action

    # If partner just made a hint move which affected at lest one completely unknown card - play it
    @staticmethod
    def play_last_revealed(observation):
        index = observation.get('last_revealed')
        if index is not -1:
            return {'action_type': 'PLAY', 'card_index': index}
        return None

    # If partner just made a hint move, play the oldest card that was referenced in the hint
    @staticmethod
    def play_latest_hint_target(observation):
        targets = observation.get('latest_hint_targets')
        if targets:
            return {'action_type': 'PLAY', 'card_index': targets[0]}
        return None

    @staticmethod
    def play_latest_hint_target_newest(observation):
        targets = observation.get('latest_hint_targets')
        if targets:
            return {'action_type': 'PLAY', 'card_index': targets[-1]}
        return None

        # If partner just made a hint move, discard the oldest card that was referenced in the hint

    @staticmethod
    def discard_latest_hint_target(observation):
        targets = observation.get('latest_hint_targets')
        if targets:
            return {'action_type': 'DISCARD', 'card_index': targets[0]}
        return None
