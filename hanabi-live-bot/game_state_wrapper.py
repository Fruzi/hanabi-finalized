...
class GameStateWrapper:
    ...
    def update_state(self, notify_msg):
        ...
        if not self.vectorizer_is_synced:
            if self.caller_is_admin:
                self.sync_vectorizer(
                    d)  # if necessary, invoke the vectorize_observation()-method with the card knowledge of
    ...
    def update_clues(self, dict_clue):
        ...
        for c in touched_cards:
            ...
            if clue['type'] == 0:
                ...
                new_rank_clue = self.card(old_color_clue, clue['value'])[
                    'rank']  # pass rank to card() function for conversion
                # update old+new clue
    ...
    def append_to_last_moves(self, dict_action, deepcopy_card_nums, scored, information_token, deal_to_player,
                             card_info_revealed):
        """
        ...
        player = get_player(dict_action)  # get_player(dict_action)
