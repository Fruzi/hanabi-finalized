...
def get_move_card_index(move, state):
    """
    ...
    if move['type'] == 'play' or move['type'] == 'discard':
        ...
        abs_card_num = move['order']
        # get target player index
        player_index = move["playerIndex"]
        hand = state.observed_hands[player_index]
        card_index = -1
        for i in range(len(hand)):
            card = hand[i]
            if card["order"] == abs_card_num:
                card_index = i
    return card_index
...
def get_move_rank(move):
    ...
    # for PLAY moves
    if move['type'] == 'play' or move['type'] == 'discard':
        rank = parse_rank_pyhanabi(rank=move['rank'])
...
def get_move_color(move):
    ...
    if move['type'] == 'clue':
        # 1 means rank clue, 0 means color clue
        if (move['clue']['type']) == 0:
...
def create_env_mock(num_players, num_colors, num_ranks, hand_size, max_information_tokens, max_life_tokens, max_moves,
                    variant):
...
def get_pyhanabi_move_mock(dict_action, state):
    ...
    # Cards in hand are ordered from oldest to newest in pyhanabi, unlike in server
    card_index = get_move_card_index(dict_action, state)
    if "target" in dict_action and "playerIndex" in dict_action:
        target_offset = get_target_offset(dict_action['playerIndex'], dict_action['target'], state.num_players)
    else:
        target_offset = -1
