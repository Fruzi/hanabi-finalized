...
LENIENT_SCORE = False
rule_based_partner = None
shared_experiment_logger = None
...
def run_one_episode(agent, environment, obs_stacker, partner, view_partner_tuples):
    ...
    Args:
      ...
      obs_stacker: Observation stacker object.
      partner: the agent with whom this agent trains
    ...
    while not is_done:
        # if isinstance(action, dict) and action["action_type"] == 'PLAY':
        #     print('Action: {} taken by player {}'.format(action, current_player))
        ...
        current_player, legal_moves, observation_vector = (
            parse_observations(observations, environment.num_moves(), obs_stacker))
        if current_player != agent_index and partner is not None:
            action = partner.act(observations['player_observations'][partner_index])
            if view_partner_tuples:
                step_number += 1
                agent.step(reward_since_last_action[current_player], current_player, legal_moves, observation_vector)
        else:
            step_number += 1
    ...
    return step_number, regular_reward, lenient_reward


def run_one_phase(agent, environment, obs_stacker, min_steps, statistics,
                  run_mode_str, partner, view_partner_tuples):
    ...
    while step_count < min_steps:
        episode_length, episode_regular_return, episode_lenient_return = run_one_episode(agent, environment,
                                                                                         obs_stacker, partner,
                                                                                         view_partner_tuples)
        statistics.append({
            '{}_episode_lengths'.format(run_mode_str): episode_length,
            '{}_episode_returns'.format(run_mode_str): episode_regular_return
        })

        step_count += episode_length
        sum_returns += episode_regular_return
        sq_sum_returns += episode_regular_return * episode_regular_return
        num_episodes += 1
    return step_count, sum_returns, num_episodes, sq_sum_returns


@gin.configurable
def run_one_iteration(agent, environment, obs_stacker,
                      iteration, training_steps, partner, view_partner_tuples,
                      evaluate_every_n=5,
    ...
    number_steps, sum_returns, num_episodes, sq_sum_returns = (
        run_one_phase(agent, environment, obs_stacker, training_steps, statistics,
                      'train', partner, view_partner_tuples))
    time_delta = time.time() - start_time
    tf.logging.info('Average training steps per second: %.2f',
                    number_steps / time_delta)

    average_return = sum_returns / num_episodes
    std_dev = math.sqrt((sq_sum_returns - (sum_returns * average_return)) / (num_episodes - 1))
    tf.logging.info('Average per episode return: %.2f', average_return)
    tf.logging.info('Average per episode return standard deviation: %.4f', std_dev)
    ...
    if evaluate_every_n is not None and iteration % evaluate_every_n == 0:
        for i in range(0, 2):
            if i == 0:
                ...
                eval_partner = rule_based_partner
            episode_steps = []
            episode_regular_rewards = []
            episode_lenient_rewards = []
            agent.eval_mode = True
            # Collect episode data for all games.
            for _ in range(num_evaluation_games):
                steps, regular, lenient = run_one_episode(agent, environment, obs_stacker, eval_partner, False)
                episode_steps.append(steps)
                episode_regular_rewards.append(regular)
                episode_lenient_rewards.append(lenient)
            # Process data
            regular_score_std_dev = np.std(episode_regular_rewards)
            regular_score_mean = np.mean(episode_regular_rewards)
            lenient_score_std_dev = np.std(episode_lenient_rewards)
            lenient_score_mean = np.mean(episode_lenient_rewards)
            mean_steps = np.mean(episode_steps)
            statistics.append({
                'eval_regular_score': regular_score_mean,
                'eval_regular_sore_std_dev': regular_score_std_dev,
                'eval_lenient_score': lenient_score_mean,
                'eval_lenient_score_std_dev': lenient_score_std_dev,
                'eval_num_steps': mean_steps

            })
            tf.logging.info('Average eval. episode length: %.2f  Return: %.2f',
                            mean_steps, lenient_score_mean)
    else:
        statistics.append({
            'eval_regular_score': -1,
            'eval_regular_sore_std_dev': -1,
            'eval_lenient_score': -1,
            'eval_lenient_score_std_dev': -1,
            'eval_num_steps': -1
        })
...
def checkpoint_experiment(experiment_checkpointer, agent, experiment_logger,
    ...
    """
    if (iteration + 1) % checkpoint_every_n == 0:
...
@gin.configurable
def run_mixed_experiment(agent,
                         environment,
                         start_iteration,
                         obs_stacker,
                         experiment_logger,
                         ...
                         self_train_ratio=2,
                         view_partner_tuples=False):
    """Runs a full experiment, spread over multiple iterations."""
    global rule_based_partner
    global shared_experiment_logger
    tf.logging.info('Beginning training...')
    # If self_train_ratio is between 0 and 1 then it means to favour mixed play, e.g. a ratio of 1/3 means play in
    # self-play every third game, while if the ratio is 3 then play in mixed-play every third game.
    formatted_sp_ratio = self_train_ratio
    if 0 < formatted_sp_ratio < 1:
        formatted_sp_ratio = 1 / formatted_sp_ratio
    if num_iterations <= start_iteration:
        tf.logging.warning('num_iterations (%d) < start_iteration(%d)',
                           num_iterations, start_iteration)
        return
    rule_based_partner = partner
    shared_experiment_logger = experiment_logger
    for iteration in range(start_iteration, num_iterations):
        start_time = time.time()
        if self_train_ratio == 0:
            tf.logging.info('Iteration %d is played with a partner', iteration)
            statistics = run_one_iteration(agent, environment, obs_stacker, iteration,
                                           training_steps, partner, view_partner_tuples)
        elif self_train_ratio == -1:
            tf.logging.info('Iteration %d is played in self-play', iteration)
            statistics = run_one_iteration(agent, environment, obs_stacker, iteration,
                                           training_steps, None, view_partner_tuples)
        elif iteration % formatted_sp_ratio == 0:
            if self_train_ratio > 1:
                tf.logging.info('Iteration %d is played with a partner', iteration)
                statistics = run_one_iteration(agent, environment, obs_stacker, iteration,
                                               training_steps, partner, view_partner_tuples)
            else:
        ...
        checkpoint_experiment(experiment_checkpointer, agent, experiment_logger,
                              iteration, checkpoint_dir, checkpoint_every_n)
        tf.logging.info('Checkpointing iteration %d took %d seconds\n', iteration,
                        time.time() - start_time)
    return agent
